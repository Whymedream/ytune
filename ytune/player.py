...
if TYPE_CHECKING:
    from disnake.types.voice import VoiceServerUpdate
...
class Filters:
    ...
    def has_preload(self) -> bool:
        return any(f for f in self._filters if f.preload is True)

    @property
    def has_global(self) -> bool:
        return any(f for f in self._filters if f.preload is False)
    ...
    def edit_filter(self, *, filter_tag: str, to_apply: Filter) -> None:
        ...
        for index, filter in enumerate(self._filters):
            if filter.tag == filter_tag:
                if type(filter) is not type(to_apply):
    ...
    def has_filter_type(self, *, filter_type: Filter) -> bool:
        return any(f for f in self._filters if type(f) is type(filter_type))

    def reset_filters(self) -> None:
        self._filters = []

    def get_preload_filters(self) -> List[Filter]:
        return [f for f in self._filters if f.preload is True]
...
class Player(VoiceProtocol):
    ...
    def __repr__(self) -> str:
        return (
            f"<player bot={self.bot} guildId={self.guild.id} "
    ...
    async def _dispatch_event(self, data: dict) -> None:
        ...
        event: YTuneEvent = getattr(events, event_type)(data, self)
